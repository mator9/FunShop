...
import ChatPasteModal from '../components/ChatPasteModal';
...
export default function ListPage() {
  ...
  const handleChatParsed = (items) => {
    setParsedItems(items);
    setShowChatPaste(false);
  };

  const handleChatItemsConfirm = async (items) => {
    await batchAddItems(id, items, userName);
    setParsedItems(null);
  };

  const handleChatImportClose = () => {
    setShowChatPaste(false);
    setParsedItems(null);
  };
    ...
    <div className="list-page">
      ...
      <main className="list-content">
        <div className="add-item-row">
          ...
          <button
            className="btn btn-chat-import"
            onClick={() => setShowChatPaste(true)}
            title="Import items from a chat message"
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      {showChatPaste && (
        <ChatPasteModal
          onParsed={handleChatParsed}
          onClose={handleChatImportClose}
        />
      )}
      ...
      {parsedItems && (
        <ParsedItemsReview
          items={parsedItems}
          onConfirm={handleChatItemsConfirm}
          onBack={() => { setParsedItems(null); setShowChatPaste(true); }}
          onClose={handleChatImportClose}
        />
      )}
    </div>
