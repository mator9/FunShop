...
app.post('/api/lists/:listId/items', async (req, res) => {
  try {
    ...
    const id = generateId(12);
    const item = await db.addItem(id, req.params.listId, name.trim(), quantity, addedBy);
...
app.patch('/api/items/:id', async (req, res) => {
  try {
    const { name, quantity, is_found, found_by } = req.body;
    const item = await db.updateItem(req.params.id, { name, quantity, is_found, found_by });
    if (!item) {
